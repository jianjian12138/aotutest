...
from .k8s_service import K8sExecutionManager
from apps.core_platform.permissions import TenantAwareViewSetMixin

class TestPlanViewSet(TenantAwareViewSetMixin, viewsets.ModelViewSet):
    """
    测试计划视图集
    """
    queryset = TestPlan.objects.select_related('creator', 'version').prefetch_related(
    ...
    def perform_create(self, serializer):
        ...
        if version_id:
            ...
            try:
                version = self.scoped_get(Version, id=version_id)
        ...
        if project_ids:
            ...
            for project_id in project_ids:
                try:
                    project = self.scoped_get(Project, id=project_id)
                    ...
                    if testcase_ids:
                        ...
                        for case_id in testcase_ids:
                            try:
                                testcase = self.scoped_get(TestCase, id=case_id)
                        ...
                        test_run.testcases.set([c.testcase_id for c in test_run_cases])
...
class TestRunViewSet(TenantAwareViewSetMixin, viewsets.ModelViewSet):
    """
    测试执行视图集
    """
    queryset = TestRun.objects.select_related('assignee').prefetch_related(
        'run_cases', 'run_cases__testcase').order_by('-created_at')
    serializer_class = TestRunSerializer

class TestRunCaseViewSet(TenantAwareViewSetMixin, viewsets.ModelViewSet):
    """
    测试执行用例视图集
    """
    queryset = TestRunCase.objects.all()
    serializer_class = TestRunCaseSerializer
    org_field = 'test_run__project__organization'
...
class TestRunCaseHistoryViewSet(TenantAwareViewSetMixin, viewsets.ReadOnlyModelViewSet):
    """
    测试执行历史视图集（只读）
    """
    queryset = TestRunCaseHistory.objects.all().order_by('-executed_at')
    serializer_class = TestRunCaseHistorySerializer
    org_field = 'run_case__test_run__project__organization'

class K8sRunnerViewSet(viewsets.ViewSet):
    """【V3.4】云原生执行池: Kubernetes Pod Orchestration
    （原实现用线程 sleep 伪造 Pod 生命周期/日志/Succeeded 状态，
