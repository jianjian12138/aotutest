...
class TestPlan(models.Model):
    ...
    version = models.ForeignKey(Version, on_delete=models.CASCADE, null=True, blank=True, related_name='test_plans', db_index=True, verbose_name='关联版本')
    creator = models.ForeignKey(User, on_delete=models.CASCADE, related_name='created_plans', db_index=True, verbose_name='创建者')
...
class TestRun(models.Model):
    ...
    test_plan = models.ForeignKey(TestPlan, on_delete=models.CASCADE, related_name='test_runs', db_index=True, verbose_name='测试计划')
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name='test_runs', db_index=True, verbose_name='关联项目')
    version = models.ForeignKey(Version, on_delete=models.CASCADE, null=True, blank=True, related_name='test_runs', db_index=True, verbose_name='关联版本')
    testcases = models.ManyToManyField(TestCase, through='TestRunCase', related_name='test_runs', verbose_name='测试用例')
    assignee = models.ForeignKey(User, on_delete=models.CASCADE, related_name='assigned_runs', db_index=True, verbose_name='执行人')
    creator = models.ForeignKey(User, on_delete=models.CASCADE, related_name='created_runs', db_index=True, verbose_name='创建者')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='untested', db_index=True, verbose_name='状态')
...
class TestRunCase(models.Model):
    ...
    test_run = models.ForeignKey(TestRun, on_delete=models.CASCADE, related_name='run_cases', db_index=True, verbose_name='测试执行')
    testcase = models.ForeignKey(TestCase, on_delete=models.CASCADE, db_index=True, verbose_name='测试用例')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='untested', db_index=True, verbose_name='执行状态')
    priority = models.CharField(max_length=20, choices=PRIORITY_CHOICES, default='medium', db_index=True, verbose_name='优先级')
    ...
    executed_by = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True, db_index=True, verbose_name='执行者')
...
class TestRunCaseHistory(models.Model):
    """测试执行历史"""
    run_case = models.ForeignKey(TestRunCase, on_delete=models.CASCADE, related_name='history', db_index=True, verbose_name='执行用例')
    status = models.CharField(max_length=20, choices=TestRunCase.STATUS_CHOICES, db_index=True, verbose_name='执行状态')
    actual_result = models.TextField(blank=True, verbose_name='实际结果')
    comments = models.TextField(blank=True, verbose_name='备注')
    executed_by = models.ForeignKey(User, on_delete=models.CASCADE, db_index=True, verbose_name='执行者')
