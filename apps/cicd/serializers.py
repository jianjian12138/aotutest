...
class CICDServerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'password': {'write_only': True},  # 密码只在写入时显示
        }
...
class CICDTriggerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CICDTrigger
        fields = ['id', 'job', 'job_name', 'trigger_type', 'trigger_type_display', 'cron_expression', 'webhook_url', 'is_active', 'created_by', 'created_by_username', 'created_at', 'updated_at']
