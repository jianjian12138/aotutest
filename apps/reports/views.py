...
from apps.core_platform.permissions import OwnedQuerySetMixin, scoped_queryset_for

class TestReportViewSet(OwnedQuerySetMixin, viewsets.ModelViewSet):
    format_kwarg = None
    # TestReport 的归属字段是 generated_by（非默认 created_by），
    # 否则 get_queryset 会按 created_by 过滤引发 FieldError 500。
    owner_field = 'generated_by'
    ...
    def analyze(self, request, pk=None):
        ...
        config = None
        if model_config_id:
            # AIModelConfig 按 created_by 自动做租户过滤；查不到回退激活配置
            config = scoped_queryset_for(
                request.user, AIModelConfig).filter(id=model_config_id).first()
        if not config:
...
"""
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    import time
                    # audit: real-impl AI 调用失败后的重试退避等待，非伪装耗时
                    time.sleep(1) # 短暂亦避让
