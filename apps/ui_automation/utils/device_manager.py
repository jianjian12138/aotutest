            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    ...
                    if len(parts) >= 2:
                        ...
                        # 尝试通过 android_id 进行设备去重 (例如 MuMu 模拟器可能同时暴露出 7555 和 5555 端口)
                        if status == 'device':
                            try:
                                aid_res = subprocess.run(['adb', '-s', device_id, 'shell', 'settings', 'get', 'secure', 'android_id'], capture_output=True, text=True, timeout=1)
                                if aid_res.returncode == 0:
                                    android_id = aid_res.stdout.strip()
                                    if android_id and android_id in seen_android_ids:
                                        # 如果物理/虚拟设备已经是同一个，跳过该端口别名
                                        print(f"Skipping duplicate device alias {device_id} (android_id: {android_id})")
                                        continue
                                    if android_id:
                                        seen_android_ids.add(android_id)
                            except Exception:
                                pass
                        ...
                        if status == 'device':
                            try:
                                ver_res = subprocess.run(['adb', '-s', device_id, 'shell', 'getprop', 'ro.build.version.release'], capture_output=True, text=True, timeout=1)
