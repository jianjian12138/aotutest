...
class UiTestCaseModuleViewSet(BaseProjectViewSet):
    ...
    def batch_update_order(self, request):
        ...
        for index, item_id in enumerate(items):
            try:
                module = UiTestCaseModule.objects.get(id=item_id)
                module.order = index
                module.save()
...
class TestCaseViewSet(BaseProjectViewSet):
    """测试用例视图集"""
    ...
    def upload_record(self, request):
        ...
        try:
            ...
            test_case = TestCase.objects.create(name=name, project=project,
                created_by=request.user, status='draft', description=
                '由 Playwright 录制生成')
            for step in steps:
    ...
    def _generate_step_log(self, step, step_result='success'):
        ...
        if step.element:
            ...
            locator_info = (
                f'{step.element.locator_strategy.name}={step.element.locator_value}'
                )
        else:
            element_name = '页面'
            locator_info = '无'
        if step.action_type == 'click':
            ...
            if step_result == 'success':
                ...
                log_parts.append(
                    f"- 获取到文本: '{mock_text}' - 耗时 {execution_time}s")
            else:
            ...
            if step_result == 'success':
                ...
                log_parts.append(
                    f'- 截图保存成功: {screenshot_name} - 耗时 {execution_time}s')
            else:
            ...
            if step.input_value:
                log_parts.append(f'- 输入值: {step.input_value}')
            log_parts.append(
                f"- 操作{'成功' if step_result == 'success' else '失败'} - 耗时 {execution_time}s"
                )
        if step.description:
        ...
        try:
            ...
            try:
                font_title = ImageFont.truetype(
                    '/System/Library/Fonts/PingFang.ttc', 40)
                font_text = ImageFont.truetype(
                    '/System/Library/Fonts/PingFang.ttc', 24)
            except:
                font_title = ImageFont.load_default()
                font_text = ImageFont.load_default()
            draw.text((40, 20), '测试步骤执行失败', fill=(255, 255, 255), font=
                font_title)
            info_y = 120
            draw.text((40, info_y), f'失败步骤: 步骤 {step_number}', fill=(50, 50,
                50), font=font_text)
            draw.text((40, info_y + 40), f'步骤说明: {step_description}', fill=
                (50, 50, 50), font=font_text)
            draw.text((40, info_y + 80),
                f"失败时间: {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}",
                fill=(50, 50, 50), font=font_text)
            browser_y = info_y + 140
            draw.rectangle([40, browser_y, width - 40, height - 40],
                outline=(200, 200, 200), width=2)
            draw.rectangle([40, browser_y, width - 40, browser_y + 40],
                fill=(200, 200, 200))
            draw.text((60, browser_y + 10), '模拟浏览器页面 - 失败截图', fill=(80, 80,
                80), font=font_text)
            error_y = browser_y + 80
            draw.text((60, error_y), '× 元素定位失败或操作执行异常', fill=(220, 53, 69),
                font=font_text)
            draw.text((60, error_y + 40), '× 请检查元素定位器是否正确', fill=(220, 53, 
                69), font=font_text)
            draw.text((60, error_y + 80), '× 或页面加载是否完成', fill=(220, 53, 69),
                font=font_text)
            buffer = io.BytesIO()
