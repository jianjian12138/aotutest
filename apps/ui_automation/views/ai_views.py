...
# 任务停止信号表：task_id -> bool，用于前端/接口请求中止正在执行的任务
STOP_SIGNALS = {}
User = get_user_model()
...
class AICaseViewSet(TenantAwareViewSetMixin, BaseProjectViewSet):
    ...
    def create_api_case(self, request):
        ...
        from apps.api_testing.models import ApiProject, ApiTestCaseStep
        try:
            # ApiProject 无 organization 字段，租户锚点为负责人 owner
            project = self.scoped_get(ApiProject, org_field='owner', id=
                project_id)
        ...
        if save_as_case:
            ...
            # 经租户校验的 project 上走关系访问创建，避免挂载到他租户项目
            api_test_case = project.test_cases.create(name=name,
                description=task_description, status='ready', created_by=
                request.user)
        ...
        def run_task():
            ...
            try:
                ...
                if ai_case.execution_mode == 'mobile':
                    ...
                    import asyncio
                    # 第六轮批次2 #157：按操作人所属组织收口在线设备，禁止跨租户抢占他组织设备
                    device = self.scoped_queryset(UiDevice, 'organization').filter(
                        status='online').first()
                    if not device:
...
class AIExecutionRecordViewSet(TenantAwareViewSetMixin, BaseProjectViewSet):
    """AI执行记录视图集"""
    ...
    def run_adhoc(self, request):
        ...
        try:
            ...
            return Response({'error': '项目不存在'}, status=status.
                HTTP_404_NOT_FOUND)
        # 经租户校验的 project 上走关系访问创建执行记录
        execution_record = project.aiexecutionrecord_set.create(
...
""")
    ...
    def run_agent_browser(self, request):
        ...
        task_description = request.data.get('task_description')
        model_config_id = request.data.get('model_config_id')
        request.data.get('enable_planning', True)
        enable_script_gen = request.data.get('enable_script_gen', True)
        request.data.get('enable_visual_diff', False)
        browser_type = request.data.get('browser_type', 'chrome')
