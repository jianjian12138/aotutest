...
class ElementGroupViewSet(BaseProjectViewSet):
    ...
    def tree(self, request):
        ...
        try:
            ...
            for po in page_objects:
                group, _ = ElementGroup.objects.get_or_create(project_id=
                    project_id, name=po.name, defaults={'description': po.
                    description or f'Auto-generated for {po.name}'})
                Element.objects.filter(project_id=project_id, page=po.name,
                    group__isnull=True).update(group=group)
            pages = Element.objects.filter(project_id=project_id,
                page__isnull=False).exclude(page='').values_list('page',
                flat=True).distinct()
            for page_name in pages:
                group, _ = ElementGroup.objects.get_or_create(project_id=
                    project_id, name=page_name, defaults={'description':
                    f'Auto-generated group for {page_name}'})
                Element.objects.filter(project_id=project_id, page=
                    page_name, group__isnull=True).update(group=group)
        except Exception as e:
