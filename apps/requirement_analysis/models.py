...
# ---- 简单熔断 + 指数退避重试（仅作用于 AIModelService.call_openai_compatible_api）----
_API_CB_LOCK = Lock()
_API_CB_FAILURES = 0
_API_CB_OPEN_UNTIL = 0.0
_API_CB_THRESHOLD = 5        # 连续失败达到该值则熔断
_API_CB_COOLDOWN = 30.0     # 熔断冷却时间（秒）
_API_MAX_RETRIES = 3
_API_BACKOFF_BASE = 1.0     # 退避基数（秒），第 n 次重试等待 base * 2**(n-1)


def _record_api_failure():
    """记录一次失败，必要时开启熔断。"""
    global _API_CB_FAILURES, _API_CB_OPEN_UNTIL
    with _API_CB_LOCK:
        _API_CB_FAILURES += 1
        if _API_CB_FAILURES >= _API_CB_THRESHOLD:
            _API_CB_OPEN_UNTIL = time.monotonic() + _API_CB_COOLDOWN
...
class RequirementDocument(models.Model):
    ...
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='uploaded', db_index=True, verbose_name='状态')
...
class GeneratedTestCase(models.Model):
    ...
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='generated', db_index=True, verbose_name='状态')
...
class AIModelConfig(models.Model):
    ...
    api_key = EncryptedTextField(verbose_name='API Key', blank=True, null=True)
...
class AIModelService:
    ...
    async def call_openai_compatible_api(config: AIModelConfig, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        # 熔断检查：熔断期内直接失败，避免对故障 LLM 持续打流量
        now = time.monotonic()
        with _API_CB_LOCK:
            if now < _API_CB_OPEN_UNTIL:
                raise Exception("AI 模型服务熔断中（circuit breaker open），请稍后重试或检查配置")
        
        global _API_CB_FAILURES
        last_exc = None
        for attempt in range(_API_MAX_RETRIES):
            try:
                # 保持 300s 超时一致，覆盖长生成任务
                async with httpx.AsyncClient(timeout=300.0) as client:
                ...
                last_exc = e
                # 只重试瞬时错误：超时 / 连接错误 / 5xx；4xx（鉴权等）直接抛出，避免无效重试
                is_transient = isinstance(e, (
                    httpx.TimeoutException, httpx.ConnectError,
                    httpx.ReadError, httpx.RemoteProtocolError,
                ))
                is_5xx = isinstance(e, httpx.HTTPStatusError) and (
                    500 <= getattr(e.response, 'status_code', 0) < 600
                )
                if not (is_transient or is_5xx):
                    _record_api_failure()
                    raise
                if attempt < _API_MAX_RETRIES - 1:
                    await asyncio.sleep(_API_BACKOFF_BASE * (2 ** attempt))
                    continue
                # 用尽重试：记录并抛出明确错误
                _record_api_failure()
                provider_name = config.get_model_type_display()
                if isinstance(e, httpx.HTTPStatusError):
                    raise Exception(f"{provider_name} API返回错误 {e.response.status_code}: {e.response.text}")
