...
class AdvancedTestRequirementAnalyzer:
    ...
    async def _preprocess_document(self, text: str) -> str:
        ...
        # 提取关键信息段落
        self._identify_document_sections(text)
...
*分析引擎版本: Advanced Test Requirement Analyzer v2.0*
    ...
    async def analyze_flowchart(self, image_path: str) -> Dict[str, Any]:
        ...
        真实读取图片并调用多模态 LLM；任何能力缺失都诚实失败，绝不返回写死的 mock。
        """
        try:
            logger.info(f"开始图文混合需求解析: {image_path}")

            # 1) 真实读取图片并校验存在/尺寸（依赖 Pillow）
            try:
                from PIL import Image
            except ImportError:
                logger.warning("analyze_flowchart: 缺少 Pillow 依赖，无法读取图片")
                return {"error": "图像解析能力不可用：缺少 OCR/视觉模型依赖（Pillow）"}

            try:
                with Image.open(image_path) as img:
                    width, height = img.size
            except Exception as e:
                logger.warning("analyze_flowchart: 图片读取失败: %s", e)
                return {"error": f"图片文件无法读取或不存在: {e}"}
            ...
            if not model_config:
                logger.warning("analyze_flowchart: 未配置任何激活的 AI 模型，无法理解图像")
                return {"error": "当前模型不支持图像理解（无可用 AI 模型配置）"}

            # 3) 将图片以 base64 作为多模态消息传入（兼容 OpenAI 视觉接口）
            import base64
            ...
            system_prompt = (
                "你是一名资深测试工程师，擅长从流程图中提取结构化测试要素。"
                "请分析图片中的流程图，输出 JSON："
                "{\"nodes\":[{\"id\":\"\",\"type\":\"\",\"text\":\"\"}], "
                "\"edges\":[{\"from\":\"\",\"to\":\"\",\"label\":\"\"}], \"rules\":[...]}"
            )
            messages = [
                {"role": "system", "content": system_prompt},
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "请分析此流程图并输出结构化 JSON。"},
            ...
            try:
                ...
                logger.warning(
                    "analyze_flowchart: 多模态 LLM 调用失败，可能该模型不支持图像理解: %s", e
                )
                return {"error": "当前模型不支持图像理解或视觉调用失败，请配置支持视觉的模型"}

            # 4) 解析结构化结果（兼容 Markdown 代码块包裹）
            ...
            if start == -1 or end == -1 or end < start:
                logger.warning("analyze_flowchart: 返回中未找到 JSON 对象")
                return {"error": "图像理解结果解析失败，模型未返回预期 JSON 结构"}
            try:
                result = json.loads(text[start:end + 1])
            except Exception as e:
                logger.warning("analyze_flowchart: 返回内容解析失败: %s", e)
                return {"error": "图像理解结果解析失败，模型未返回预期 JSON 结构"}
            if not isinstance(result, dict):
                return {"error": "图像理解结果解析失败，返回的不是 JSON 对象"}
