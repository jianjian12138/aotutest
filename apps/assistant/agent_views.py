...
class AgentExecuteView(APIView):
    async def post(self, request, *args, **kwargs):
        ...
        from apps.assistant.models import AgentProfile
        ...
        if override_skill_ids:
            # AgentSkill 无租户字段：改为通过已校验父对象 profile 的 skills 关系过滤，
            # 覆盖范围只能在该 Agent 已绑定的技能内选择，杜绝任意 ID 挑选执行代码
            skills_to_use = profile.skills.filter(id__in=override_skill_ids, is_active=True)
...
class AgentExecuteStreamView(APIView):
    ...
    async def post(self, request, *args, **kwargs):
        ...
        if override_skill_ids:
            # AgentSkill 无租户字段：通过已校验父对象 profile 的 skills 关系过滤（同步视图同理）
            skills_to_use = await sync_to_async(
                lambda: list(profile.skills.filter(id__in=override_skill_ids, is_active=True)))()
