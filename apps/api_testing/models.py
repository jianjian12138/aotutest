...
class ApiProject(models.Model):
    ...
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, verbose_name='项目状态', db_index=True)
    start_date = models.DateField(null=True, blank=True, verbose_name='开始日期')
    end_date = models.DateField(null=True, blank=True, verbose_name='结束日期')
    owner = models.ForeignKey(User, on_delete=models.CASCADE, related_name='owned_api_projects', verbose_name='负责人', db_index=True)
...
class ApiTestCaseModule(models.Model):
    """API测试用例模块模型"""
    project = models.ForeignKey(ApiProject, on_delete=models.CASCADE, related_name='test_case_modules', verbose_name='所属项目', db_index=True)
...
class ApiTestCase(models.Model):
    ...
    project = models.ForeignKey(ApiProject, on_delete=models.CASCADE, related_name='test_cases', verbose_name='所属项目', db_index=True)
    ...
    priority = models.CharField(max_length=10, choices=PRIORITY_CHOICES, default='medium', verbose_name='优先级', db_index=True)
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='created_api_test_cases', verbose_name='创建人', db_index=True)
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间', db_index=True)
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
...
class ApiTestCaseStep(models.Model):
    ...
    test_case = models.ForeignKey(ApiTestCase, on_delete=models.CASCADE, related_name='steps', verbose_name='测试用例', db_index=True)
...
class RequestHistory(models.Model):
    ...
    executed_by = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='执行者', db_index=True)
...
class TestSuiteRequest(models.Model):
    ...
    test_suite = models.ForeignKey(TestSuite, on_delete=models.CASCADE, verbose_name='测试套件', db_index=True)
...
class TestSuiteTestCase(models.Model):
    """测试套件中的测试用例关联模型"""
    test_suite = models.ForeignKey(TestSuite, on_delete=models.CASCADE, verbose_name='测试套件', db_index=True)
    test_case = models.ForeignKey(ApiTestCase, on_delete=models.CASCADE, verbose_name='测试用例', db_index=True)
    order = models.IntegerField(default=0, verbose_name='执行顺序')
    enabled = models.BooleanField(default=True, verbose_name='是否启用')
...
class ScheduledTask(models.Model):
    ...
    # 状态管理
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='ACTIVE', verbose_name='任务状态', db_index=True)
...
class TaskExecutionLog(models.Model):
    ...
    task = models.ForeignKey(ScheduledTask, on_delete=models.CASCADE, related_name='execution_logs',
                             verbose_name='关联任务')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='PENDING', verbose_name='执行状态', db_index=True)
    start_time = models.DateTimeField(null=True, blank=True, verbose_name='开始时间')
    end_time = models.DateTimeField(null=True, blank=True, verbose_name='结束时间')
    ...
    error_message = models.TextField(blank=True, verbose_name='错误信息')
    executed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, verbose_name='执行者', db_index=True)
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间', db_index=True)
...
class ApiImportTask(models.Model):
    """API导入任务模型"""
    STATUS_CHOICES = [
        ('pending', '待处理'),
        ('running', '处理中'),
    ...
    project = models.ForeignKey(ApiProject, on_delete=models.CASCADE, related_name='import_tasks', verbose_name='所属项目')
    ...
    file_format = models.CharField(max_length=20, choices=FORMAT_CHOICES, verbose_name='文件格式')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending', verbose_name='任务状态')
    error_message = models.TextField(blank=True, verbose_name='错误信息')
    imported_count = models.IntegerField(default=0, verbose_name='导入成功数量')
    ...
    class Meta:
        db_table = 'api_import_tasks'
        verbose_name = 'API导入任务'
        verbose_name_plural = 'API导入任务'
        ordering = ['-created_at']
        indexes = [
...
class ApiTestCaseExecution(models.Model):
    ...
    test_case = models.ForeignKey(ApiTestCase, on_delete=models.CASCADE, related_name='executions', verbose_name='测试用例', db_index=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, verbose_name='执行状态', db_index=True)
    ...
    executed_by = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='执行者', db_index=True)
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='执行时间', db_index=True)
