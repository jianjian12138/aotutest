...
class TestExecutionViewSet(TenantAwareViewSetMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def generate_rca(self, request, pk=None):
        ...
        threading.Thread(target=RCAService.analyze_api_execution, args=(execution.id,)).start()
...
class OperationLogViewSet(TenantAwareViewSetMixin, viewsets.ReadOnlyModelViewSet):
    """操作日志视图集"""
    ...
    ordering = ['-created_at']
    org_field = 'user'  # 第六轮批次2：OperationLog 仅含 user 外键，按操作者收敛

    def get_queryset(self):
        # 第六轮批次2：接入统一租户隔离 —— 操作日志按操作者收敛，非本人不可见（管理员全量）
        return self._apply_tenant_scope(OperationLog.objects.all()).order_by('-created_at')
