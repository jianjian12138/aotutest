...
class NotificationConfigDetailSerializer(serializers.ModelSerializer):
    """通知配置详情序列化器（webhook_url/secret 掩码输出，write_only 语义——不回读明文，D2 P0 修复）"""
    ...
    def get_webhook_bots(self, obj):
        """获取webhook机器人列表（掩码，不含 webhook_url 明文与 secret）"""
        from apps.notifications.serializers import mask_webhook_bots
        masked = mask_webhook_bots(obj.webhook_bots)
        return [dict(v, type=k) for k, v in masked.items()]


class NotificationLogDetailSerializer(PIIMaskMixin, serializers.ModelSerializer):
    """通知日志详情序列化器"""
