...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # 创建或获取默认用户（口令从环境变量 SEED_ADMIN_PASSWORD 读取，
        # 缺失时生成随机强口令并仅在控制台输出一次，禁止硬编码弱口令）
        ...
        if created or not user.has_usable_password():
            import os
            import secrets
            pwd = os.environ.get('SEED_ADMIN_PASSWORD') or secrets.token_urlsafe(16)
            user.set_password(pwd)
            user.save()
            self.stdout.write(self.style.WARNING(
                f'已为 admin 设置口令（请妥善保存，或预置环境变量 SEED_ADMIN_PASSWORD）：{pwd}'))
