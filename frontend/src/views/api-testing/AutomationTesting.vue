  ...
  <BasePage title="自动化测试">
    ...
    >
      <div v-if="currentExecution" class="execution-result-content">
        ...
        <div class="rca-info" v-if="currentExecution.status === 'failed' || currentExecution.status === 'FAILED'" style="margin-bottom: 20px; border: 1px solid #e1f3d8; border-radius: 4px; padding: 15px; background-color: #f0f9eb;">
          <div class="error-header" style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">
            <h3 style="margin: 0; color: #67c23a;">🤖 AI 根因诊断方案</h3>
            <el-tag :type="currentExecution.ai_rca_status === 'SUCCESS' ? 'success' : (currentExecution.ai_rca_status === 'ANALYZING' ? 'warning' : 'info')">
              {{ currentExecution.ai_rca_status || 'PENDING' }}
            </el-tag>
            <el-button 
              v-if="currentExecution.ai_rca_status !== 'ANALYZING'" 
              size="small" type="success" plain 
              @click="generateRCA(currentExecution.id)"
              :loading="isGeneratingRCA">
              ✨ AI 智能推演
            </el-button>
          </div>
          
          <div class="error-content" v-if="currentExecution.ai_rca_status === 'SUCCESS'" style="background: white; color: #303133; border: 1px solid #e4e7ed; padding: 10px; border-radius: 4px;">
            <pre style="margin: 0; font-family: inherit; white-space: pre-wrap; word-break: break-all;">{{ currentExecution.ai_rca_result }}</pre>
          </div>
          <div v-else-if="currentExecution.ai_rca_status === 'ANALYZING'" style="padding: 20px; text-align: center; color: #909399;">
             <p>✨ 云端 AI 专家正在读取执行堆栈寻找崩溃根因，请稍候...</p>
          </div>
          <div v-else style="padding: 10px; color: #909399;">
            <p>点击上方按钮让 AI 帮助您分析此条失败流水线记录并生成自愈方案。</p>
          </div>
        </div>
...
const isGeneratingRCA = ref(false)
...
const viewExecutionDetail = (execution) => {
  ...
  showExecutionDialog.value = true
  if (execution.ai_rca_status === 'ANALYZING') {
    startPolling(execution.id)
  }
}

const generateRCA = async (id) => {
  try {
    isGeneratingRCA.value = true
    await api.post(`/api-testing/test-executions/${id}/generate-rca/`)
    ElMessage.success('诊断请求已提交，请等待 AI 分析完毕')
    currentExecution.value.ai_rca_status = 'ANALYZING'
    startPolling(id)
  } catch(e) {
    ElMessage.error('触发诊断失败')
  } finally {
    isGeneratingRCA.value = false
  }
}
...
const startPolling = (id) => {
  ...
  pollingTimer = setInterval(async () => {
    if (!showExecutionDialog.value || currentExecution.value?.id !== id) {
      clearInterval(pollingTimer)
      return
    }
    try {
      const res = await api.get(`/api-testing/test-executions/${id}/`)
      const exec = res.data
      if (exec) {
         currentExecution.value.ai_rca_status = exec.ai_rca_status
         currentExecution.value.ai_rca_result = exec.ai_rca_result
         if (exec.ai_rca_status !== 'ANALYZING') {
           clearInterval(pollingTimer)
           pollingTimer = null
         }
      }
