<template>
  <BasePage title="UI元素管理">
        ...
        <div v-else class="element-detail">
          ...
          <div class="element-header">
            <div class="element-info">
              ...
              <el-button v-if="selectedElement.id" size="small" type="success" plain @click="aiHealElement" :loading="healing">
                <el-icon><Wand /></el-icon> ✨ AI 智能诊断修复
              </el-button>
            </div>
          </div>
    ...
    <!-- AI 自愈推荐对话框 -->
    <el-dialog v-model="showAiHealDialog" title="🤖 AI Locator 自愈诊断" width="700px">
      <div v-loading="healing" element-loading-text="大语言模型正在分析页面语义与定位树...">
        <div v-if="aiSuggestions.length > 0">
          <p style="color: #67c23a; margin-bottom: 20px;">分析完成！诊断引擎已生成以下 {{ aiSuggestions.length }} 种极具鲁棒性的替换定位策略：</p>
          <el-table :data="aiSuggestions" border>
            <el-table-column prop="strategy" label="策略" width="100">
              <template #default="scope">
                <el-tag>{{ scope.row.strategy }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="value" label="定位表达式" min-width="200" />
            <el-table-column prop="description" label="诊断理由" min-width="200" />
            <el-table-column label="操作" width="110">
              <template #default="scope">
               <el-button size="small" type="primary" link @click="applyAiSuggestion(scope.row)">一键采纳</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
        <div v-else-if="!healing">
          <el-empty description="系统暂无推荐的自愈策略"></el-empty>
        </div>
      </div>
    </el-dialog>
...
import {
  ...
  Folder, Document as DocumentIcon, Operation, DocumentCopy, ArrowDown, MagicStick
...
// AI 自愈相关
const showAiHealDialog = ref(false)
const healing = ref(false)
const aiSuggestions = ref([])

const aiHealElement = async () => {
  if (!selectedElement.value?.id) return
  showAiHealDialog.value = true
  healing.value = true
  aiSuggestions.value = []
  try {
    const res = await request.post(`/ui-automation/elements/${selectedElement.value.id}/ai-heal/`)
    if (res.data && res.data.suggestions) {
      aiSuggestions.value = res.data.suggestions
    }
  } catch (error) {
    ElMessage.error('AI 元素自愈请求验证失败')
  } finally {
    healing.value = false
  }
}

const applyAiSuggestion = (row) => {
  const strategy = locatorStrategies.value.find(s => s.name.toLowerCase() === row.strategy.toLowerCase() || s.name.toLowerCase() === row.strategy.toLowerCase() + ' selector')
  if (strategy) {
    selectedElement.value.locator_strategy_id = strategy.id
  }
  selectedElement.value.locator_value = row.value
  showAiHealDialog.value = false
  ElMessage.success('已自动应用推荐的自愈规则，正在准备覆盖。请记得点击保存！')
}
