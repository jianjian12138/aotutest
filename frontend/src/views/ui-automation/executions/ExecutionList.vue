  ...
  <BasePage title="执行记录">
    ...
    >
      <div v-if="currentExecution" class="execution-detail">
        ...
        <div class="rca-info" v-if="currentExecution.status === 'failed' || currentExecution.status === 'FAILED' || currentExecution.error_message" style="margin-top: 20px; border: 1px solid #e1f3d8; border-radius: 4px; padding: 15px; background-color: #f0f9eb;">
          ...
          <div class="error-content" v-if="currentExecution.ai_rca_status === 'SUCCESS'" style="background: white; color: #303133; border: 1px solid #e4e7ed;">
...
const viewDetail = (row) => {
  ...
  if (row.ai_rca_status === 'ANALYZING') {
    startPolling(row.id)
  }
}
...
const generateRCA = async (id) => {
  try {
    ...
    await request.post(`/ui-automation/executions/${id}/generate-rca/`)
...
const startPolling = (id) => {
  ...
  pollingTimer = setInterval(async () => {
    ...
    try {
      const res = await request.get(`/ui-automation/executions/${id}/`)
      const exec = res.data
      if (exec) {
