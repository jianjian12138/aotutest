<template>
  <BasePage title="MQTT 测试">
    ...
    <el-table :data="tasks" v-loading="loading">
      ...
      <el-table-column label="Broker">
        <template #default="scope">
          {{ scope.row.config_snapshot?.mqtt_broker }}:{{ scope.row.config_snapshot?.mqtt_port }}
          <el-tag size="small" v-if="scope.row.config_snapshot?.mqtt_use_tls" type="success">TLS</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" title="新建 MQTT 测试" width="600px">
      <el-form :model="form" label-width="120px">
        <el-form-item label="任务名称">
          <el-input v-model="form.name" />
        </el-form-item>
        <el-form-item label="Broker">
          <el-input v-model="form.mqtt_broker" placeholder="例如: broker.emqx.io" />
        </el-form-item>
        <el-form-item label="Port">
          <el-input-number v-model="form.mqtt_port" :min="1" :max="65535" />
        </el-form-item>
        <el-form-item label="TLS 加密">
          <el-switch v-model="form.mqtt_use_tls" />
        </el-form-item>
        
        <el-divider content-position="left">发布设置</el-divider>
        <el-form-item label="发布 Topic">
          <el-input v-model="form.mqtt_topic" placeholder="test/topic" />
        </el-form-item>
        <el-form-item label="消息内容">
          <el-input type="textarea" v-model="form.mqtt_message" placeholder="Hello MQTT" />
        </el-form-item>
        <el-row>
          <el-col :span="12">
            <el-form-item label="QoS">
              <el-select v-model="form.mqtt_qos">
                <el-option :label="'0 - At most once'" :value="0" />
                <el-option :label="'1 - At least once'" :value="1" />
                <el-option :label="'2 - Exactly once'" :value="2" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
             <el-form-item label="发送次数">
               <el-input-number v-model="form.mqtt_msg_count" :min="1" />
             </el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="间隔 (秒)" v-if="form.mqtt_msg_count > 1">
           <el-input-number v-model="form.mqtt_interval" :min="0.1" :step="0.1" />
        </el-form-item>

        <el-divider content-position="left">订阅设置 (可选)</el-divider>
        <el-form-item label="订阅 Topic">
          <el-input v-model="form.mqtt_subscribe_topic" placeholder="留空不订阅" />
          <div style="font-size: 12px; color: #999">设置后将等待接收响应消息</div>
        </el-form-item>
...
const form = reactive({
  ...
  mqtt_use_tls: false,
  mqtt_message: 'Hello from Platform',
  mqtt_qos: 0,
  mqtt_msg_count: 1,
  mqtt_interval: 1.0,
...
const fetchTasks = async () => {
  loading.value = true
  try {
    const res = await request.get('/special-testing/tasks/', { params: { protocol: 'MQTT' } })
