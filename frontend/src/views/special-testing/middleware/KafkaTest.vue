<template>
  <BasePage title="Kafka 工具">
    ...
    <el-table :data="tasks" v-loading="loading">
      ...
      <el-table-column label="配置信息">
        <template #default="scope">
          <div>Servers: {{ scope.row.config_snapshot?.kafka_servers }}</div>
          <div>Topic: {{ scope.row.config_snapshot?.kafka_topic }}</div>
          <div>模式: {{ scope.row.config_snapshot?.kafka_test_mode }}</div>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" title="新建 Kafka 测试" width="600px">
      <el-form :model="form" label-width="120px">
        <el-form-item label="任务名称">
          <el-input v-model="form.name" />
        </el-form-item>
        <el-form-item label="Servers">
          <el-input v-model="form.kafka_servers" placeholder="localhost:9092" />
        </el-form-item>
        <el-form-item label="Topic">
          <el-input v-model="form.kafka_topic" placeholder="test-topic" />
        </el-form-item>
        
        <el-form-item label="模式">
          <el-radio-group v-model="form.kafka_test_mode">
            <el-radio label="PRODUCER">生产者 (发送消息)</el-radio>
            <el-radio label="CONSUMER">消费者 (接收消息)</el-radio>
          </el-radio-group>
        </el-form-item>

        <template v-if="form.kafka_test_mode === 'PRODUCER'">
           <el-form-item label="Message">
             <el-input type="textarea" v-model="form.kafka_message" placeholder="Hello Kafka" />
           </el-form-item>
           <el-form-item label="Key">
             <el-input v-model="form.kafka_msg_key" placeholder="Optional Key" />
           </el-form-item>
           <el-form-item label="数量">
             <el-input-number v-model="form.kafka_msg_count" :min="1" />
           </el-form-item>
        </template>

        <template v-else>
           <el-form-item label="Consumer Group">
             <el-input v-model="form.kafka_consumer_group" placeholder="Optional Group ID" />
           </el-form-item>
        </template>
...
const form = reactive({
  ...
  kafka_test_mode: 'PRODUCER',
  kafka_message: 'Hello Kafka',
  kafka_msg_key: '',
  kafka_msg_count: 1,
...
const fetchTasks = async () => {
  loading.value = true
  try {
    const res = await request.get('/special-testing/tasks/', { params: { protocol: 'KAFKA' } })
