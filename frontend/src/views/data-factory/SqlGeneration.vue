<template>
  <BasePage title="SQL 生成">
    <template #actions>
      <el-button type="success" size="small" @click="aiDataExplorerAction" :loading="aiExplorerLoading" plain>
...
import { ChatDotRound, VideoPlay, Download, Document, Timer, Star, MagicStick } from '@element-plus/icons-vue'
import {
  getVannaConfigs,
  generateSql,
  getSqlGenerations,
...
// 极速 AI 数据探索 (V2.3 Text-to-SQL)
const aiExplorerLoading = ref(false)

const aiDataExplorerAction = async () => {
  if (!naturalLanguage.value.trim()) {
    ElMessage.warning('请输入您的数据探索需求！')
    return
  }
  
  aiExplorerLoading.value = true
  errorMessage.value = ''
  
  try {
    const res = await dataExplorerChat({
      project_id: saveQueryForm.value.project_id || null,
      message: naturalLanguage.value,
      auto_execute: false
    })
    ...
    if (res.data.status === 'FAILED') {
      ...
      activeTab.value = 'sql'
      ElMessage.success('✨ AI 数据探索成功！已生成高精度极速 SQL')
    }
  } catch (error) {
    errorMessage.value = error.response?.data?.error || 'AI 探索引擎超时或断开连接'
    generatedSql.value = ''
    console.error('AI Data Explorer Error:', error)
    ElMessage.error(errorMessage.value)
  } finally {
    aiExplorerLoading.value = false
  }
}
